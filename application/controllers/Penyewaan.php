...
class Penyewaan extends CI_Controller {
    ...
    public function addPenyewaan()
    {
        ...
        $tgl1 = $this->input->post('tgl_pinjam');
        $tgl2 = $this->input->post('tgl_selesai');
        $id_user = $this->session->id_user; 

        $cektgl = $this->sewa->cektgl($tgl1, $tgl2, $id_kenda);
        // print_r($cektgl);
        // exit;
        if($cektgl->num_rows() == 1){
            $row = $cektgl->row();
            $tgl = $row->tgl_selesai;
            $this->session->set_flashdata('failed', 'Gagal. Silahkan Sewa Bus Setelah Tanggal '. $tgl);
            redirect('Pengguna');
        }else{
